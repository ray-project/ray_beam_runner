...
class RayFnApiRunnerTest(unittest.TestCase):
    ...
    def test_multimap_multiside_input(self):
        ...
        with self.create_pipeline() as p:
            ...
            assert_that(
                ...
                | "second map"
                >> beam.Map(
                    lambda k, d, l: (k[0], sorted(d[k[0]]), sorted([e[1] for e in l])),
                    beam.pvalue.AsMultiMap(side),
                    beam.pvalue.AsList(side),
                ),
    ...
    def _run_pardo_state_timers(self, windowed, key_type=None):
        ...
        def is_buffered_correctly(actual):
            ...
            self = RayFnApiRunnerTest("__init__")
    ...
    def test_pardo_dynamic_timer(self):
        class DynamicTimerDoFn(beam.DoFn):
            ...
            def process(
                self, element, dynamic_timer=beam.DoFn.TimerParam(dynamic_timer_spec)
            ):
    ...
    def test_sdf(self):
        class ExpandingStringsDoFn(beam.DoFn):
            def process(
                self,
                element,
                restriction_tracker=beam.DoFn.RestrictionParam(ExpandStringsProvider()),
            ):
    ...
    def test_metrics(self, check_gauge=True):
        ...
        t1, t2 = res.metrics().query(beam.metrics.MetricsFilter().with_name("counter"))[
            "counters"
        ]
        self.assertEqual(t1.committed + t2.committed, 6)

        (dist,) = res.metrics().query(
            beam.metrics.MetricsFilter().with_name("distribution")
        )["distributions"]
        self.assertEqual(
        ...
        if check_gauge:
            (gaug,) = res.metrics().query(
                beam.metrics.MetricsFilter().with_name("gauge")
            )["gauges"]
    ...
    def test_register_finalizations(self):
            ...
            def process(
                ...
                if restriction_tracker.try_claim(
                    ...
                    bundle_finalizer.register(
                        lambda: event_recorder.record("finalized")
                    )
                    # We sleep here instead of setting a resume time since
                    # the resume time doesn't need to be honored.
                    time.sleep(1)
                    restriction_tracker.defer_remainder()
        ...
        with self.create_pipeline() as p:
            max_retries = 100
            res = (
                p | beam.Create([max_retries]) | beam.ParDo(FinalizableSplittableDoFn())
            )
    ...
    def test_sdf_synthetic_source(self):
        ...
        source_descriptions = [
            ...
            for i in range(0, num_source_description)
        ]
        total_num_records = 0
...
class RayRunnerMetricsTest(unittest.TestCase):
        ...
        for mi in mon_infos:
            if has_urn_and_labels(mi, urn, labels):
                (
                    ...
                    extracted_min,
                    extracted_max,
                ) = monitoring_infos.extract_distribution(mi)
                increment = 1
                if sum is not None:
    ...
    def test_element_count_metrics(self):
        ...
        pardo = "StepThatDoesTwoOutputs" >> beam.ParDo(
            GenerateTwoOutputs()
        ).with_outputs("SecondOutput", "ThirdOutput", main="FirstAndMainOutput")
        ...
        try:
            ...
            self.assert_has_counter(
                counters, monitoring_infos.ELEMENT_COUNT_URN, labels, num_source_elems
            )
    ...
    def test_non_user_metrics(self):
        ...
        for urn in urns:
            ...
            assert_counter_exists(
                all_metrics_via_montoring_infos, namespace, name, step="Create/Impulse"
            )
            assert_counter_exists(
    ...
    def test_progress_metrics(self):
        ...
        _ = (
            p
            | beam.Create([0, 0, 0, 5e-3 * DEFAULT_SAMPLING_PERIOD_MS], reshuffle=False)
            | beam.Map(time.sleep)
...
class RayRunnerSplitTest(unittest.TestCase):
    ...
    def run_sdf_checkpoint(self, is_drain=False):
        ...
        self.run_sdf_split_pipeline(
            split_manager, elements, element_counter, expected_groups, is_drain=is_drain
        )
